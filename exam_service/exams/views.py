        ...
        try:
            try:
                ...
                try:
                    ...
                    details = str(e)
                return Response({"error": "User service error", "detail": details}, status=502)
...
class StudentAssignedExamsView(APIView):
    def get(self, request):
        ...
        try:
            try:
                ...
                try:
                    ...
                    details = str(e)
                return Response({"error": "Student GRPC error", "detail": details}, status=status.HTTP_502_BAD_GATEWAY)
            if not getattr(student_response, 'student', None):
                return Response({"error": "Only students can view assigned exams."}, status=status.HTTP_403_FORBIDDEN)
        finally:
            try:
                user_client.close()
            except Exception:
        ...
        client = ExamGRPCClient()
        try:
            response = client.get_exams_by_student(student_response.student.student_id)
            exams = [
                {
            ...
            try:
                ...
                details = str(e)
            return Response({"error": "Failed to fetch exams", "detail": details}, status=status.HTTP_502_BAD_GATEWAY)
        except Exception as e:
            return Response({"error": "Failed to fetch exams", "detail": str(e)}, status=status.HTTP_500_INTERNAL_SERVER_ERROR)
    ...
    def post(self, request):
        ...
        try:
            try:
                ...
                return Response({"error": "Student GRPC error", "detail": details}, status=status.HTTP_502_BAD_GATEWAY)
            if not getattr(student_response, 'student', None):
                return Response({"error": "Only students can attempt exams"}, status=status.HTTP_403_FORBIDDEN)
        finally:
            try:
                user_client.close()
            except Exception:
